...
(async () => {
  try {
    ...
    if (!year) return console.log('No academic_years found');
    const cls = await dbGet('SELECT id, name FROM classes ORDER BY id LIMIT 1');
    if (!cls) return console.log('No classes found');
    ...
    console.log(JSON.stringify({ year: {id: yearId, year: year.year}, class: {id: classId, name: cls.name}, terms, courses, students, grades }, null, 2));
    db.close();
  } catch (err) {
